import React from 'react';
import { useNavigation } from '@react-navigation/native';
import { StyleSheet, Text,Image, TouchableOpacity, SafeAreaView} from 'react-native';
import { TextInput } from 'react-native-gesture-handler';

const confirmCall = ({navigation}) => {
  
  return (
      <SafeAreaView style = {styles.container}>

        <SafeAreaView style = {styles.navBar}>
          <TouchableOpacity
           onPress={() => navigation.navigate('CallAmbulanceScreen')}>
            <Image 
              source= {{
                uri:'data:image/png;base64,'
              }}
              style={{width: 45, height: 45, marginHorizontal: 12}} 
              />
          </TouchableOpacity>
          <Text style={styles.navText}>Book A Lance</Text>
          <Image source={{
              uri: 'data:image/png;base64,'
          }}
          style={styles.hamburgerMenu}/>
        </SafeAreaView> 
          
        <SafeAreaView style = {styles.detailsInputContainer}>
          <SafeAreaView style = {styles.locationInfo}>
            <Text>An ambulance is on the way from:</Text>
            <Text style = {{margin:12, padding:10}}>(Get hospital name)Veterans Memorial Medical Center</Text>
            <Text style = {{marginLeft: 25}}>Your Location: (getUserLocation) </Text> 
          </SafeAreaView>

          <SafeAreaView style = {styles.locationDisplay}>
            
            <Image source={{
                uri: 'https://i.imgur.com/kAnZFA5.png'}}
                style={styles.locationImage}/> 
          </SafeAreaView>
        </SafeAreaView>

        <SafeAreaView style={styles.continueButtonContainer}>
          <TouchableOpacity style={styles.continueButton} onPress={() => navigation.navigate('trackAmbulanceScreen')}>
            <Text style={styles.continueButtonText}> Place Call </Text>
          </TouchableOpacity>
        </SafeAreaView>
          
      </SafeAreaView>
      
      
            
  );
}
export default confirmCall;


const styles = StyleSheet.create({

  container: {
    flex:1,
    flexDirection:'column',
    
    backgroundColor: '#ffff',
    justifyContent: 'center',
    alignItems: 'center',
    alignContent: 'center'
  },
  //Header
  navBar:{
    flex: 0.3,
    flexDirection: 'row',
    flexWrap: 'wrap',

    alignItems: 'center',
    alignContent: 'right',
    
    backgroundColor: '#FD5E5E',
    width: '100%',
    height: '8%',

    marginBottom: 10
    
    
  },
  navText: {
    color: '#ffffff',
    textAlign: 'left',

    fontSize: 34,
    fontFamily: 'Port Lligat Sans',
  },

  hamburgerMenu:{
    position: 'absolute',
    right: 16,
    width: 45, 
    height: 45, 
    
  },
//Content 
  detailsInputContainer:{
    flex: 2,
    flexWrap: 'wrap',
    backgroundColor: '#ffff',
    justifyContent: 'flex-start',
    alignItems: 'center',
    alignContent: 'center'
  },

  locationInfo:{
    flex: 0.3,
    height: '30%',

    backgroundColor: '#ffff',
    justifyContent: 'flex-start',
    alignContent: 'center',

    margin: 20
  },
 
  locationDisplay:{
    flex: 0.5,
    flexWrap: 'wrap',
    flexDirection: 'column',
    justifyContent: 'flex-start',
    
    width: '100%',
    height:  375,

    margin: 15,
    
  },

  locationImage:{

    width: '100%',
    height: '100%',
    margin: 5,
    padding: 5

  },

  //Footer button
  continueButtonContainer:{
    flex: 0.1,
    position: 'absolute',
    bottom: 20,
    alignSelf: 'center',
  },
  continueButton:{
    backgroundColor: '#FD5E5E',
    borderRadius: 10,
    width: 200,
    height: 50,
    alignItems: 'center',
    justifyContent: 'center',
  },
  continueButtonText: {
    color: '#fff',
    fontSize: 18,
    fontWeight: 'bold',
  },


});
